...
**纯桌面应用（PySide6 原生 GUI）** —— 无浏览器、无 localhost，双击即真正的软件窗口。无任何密钥也能以「演示模式」跑通整条链路（内置 2000+ 条方言词库兜底）。
...
- 📚 **河南方言词库**：内置 **2000+** 条河南方言词条（`dialect_dict.json` + `dialect_dict_extra.json` + `dialect_dict_m2.json` + `dialect_dict_m3.json` 多包聚合，去重后 2168 条），`pypinyin` 注音，覆盖称谓 / 饮食 / 动作 / 身体 / 农事 / 居家 / 自然 / 动物植物 / 衣物 / 量词 / 俗语惯用 / 情绪 / 虚词等语义分类。
